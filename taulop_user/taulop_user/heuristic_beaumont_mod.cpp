...
/* Return the number of processes per column combinations ordered */
void BeaumontHeuristicMod::agrupate (int *v, int c, int C, long int &n, Group *agr[]) {
...
void BeaumontHeuristicMod::agrupation_nr (int *v, int c, int C, long int &n) {
...
BeaumontHeuristicMod::BeaumontHeuristicMod (Algorithm *alg) {
    this->alg = alg;
}


BeaumontHeuristicMod::~BeaumontHeuristicMod () {
    
}


Arrangement ** BeaumontHeuristicMod::apply (Arrangement *r) {
    ...
    // 2.1. Put the r value. It seems to contain the number of process minus the last column
    ...
    // 3.1. Put processes (duplicate) in a vector
    int P = r->getP();
    Rectangle *v_proc[P];
    ...
    // 3.1.2. Order rectangles by process node, and area (bubble sort). This is the
    //        main modification of the Beaumont algorithm.
    for (p_nr = P-1; p_nr >= 1; p_nr--) {
        for (int j = 0; j < p_nr; j++) {
            
            int node_j   = v_proc[j]->getNode();
            int node_sig = v_proc[j+1]->getNode();
...
long int BeaumontHeuristicMod::combinations (Arrangement *r) {
