...
At this point, if you have a set of parameter files generated by mpiblib you should transform them to the tau-lop format to be used later. The code of tau-lop library comes with a Python utility to convert files, just open it (~/libtaulop/params/transform/transform_params.ipynb) in a Jupyter Notebook session, configure and run. You will get taulop ready to use parameter files.
