...
//
//  suara2d_grid_map.cpp
//  Find optimal SUARA2D grid and algorithm numerically (considering mapping)
//
...
struct grid_2d_t {
   ...
   bool operator<(const grid_2d_t& d) const {
       if (r < d.r)
           return true;
       else if (r == d.r)
           return c < d.c;
...
void grid_to_comm (grid_2d_t d, int dim, int n, int p, int *ranks) {
   /*
    * INPUT
    *   d: grid (e.g. (3,4)
    *   dim: dimension number (0, for rows and 1 for cols)
    *   n: number of row or col to get ranks
...
/*
 ...
 3. Main loop of the algorithm:
      for every valid process grid arrangement d=(dr,dc) in dims_arrangement:
      ...
      3.3. Obtain the optimum algorithm for a column of processes
         ...
         b) Invoke every algorithm and get that of minimum cost.
      3.4. Utilize a map to store the minimum values for rows and columns.
         Key:    d=(dr,dc)
         Value:  t_row+t_col, alg_row, alg_col (sum of times, algorithm for rows and algorithm for cols)
 ...
 for d in dims_arrangments:
   ...
   d=(1,12)
   ROWS: 1 row with 12 processes
         Create communicator with P=12 (map is inherited from world)
         Execute algorithms and get that of minimum cost.
   COLS: 12 cols with 1 processes
         Create a communicator with P=1 (map is inherited from world)
         Execute algorithms and get that of minimum cost.
   map[(1,12)] = {t_r + t_c, a_r, a_c}
...
Optimal_t SUARA2D_find_optimals_numerically (int P, int m, int ms, string algorithms[] /* , H */, int Q, int M, Map mapping, OpType op) {
   ...
   map<grid_2d_t, t_algs> t_algs_map;
   ...
   for (const auto& d : dims_arrangements) {
      ...
      /* 3.4. Store values for the current valid arrangement:
       *
       ...
       *
       */
      t_algs_map[d] = {t_r_min + t_c_min, a_r_min, a_c_min};
   ...
   for (const auto& pair : t_algs_map) {
      ...
      if (verbose) {
         cout << "d = (" << pair.first.r << "," << pair.first.c << ") = {" << algorithms[pair.second.a_r] << "," << algorithms[pair.second.a_c] << ") -> " << pair.second.t << "}" << endl;
      }
      
      if (pair.second.t < t_min) {
         t_min = pair.second.t;
         key_min = {pair.first.r, pair.first.c};
      }
   }
   ...
   if (verbose) {
      cout << endl << "** Optimum 2D process grid arrangement: " << endl;
      cout << "d = (" << key_min.r << "," << key_min.c << ") = {(" << algorithms[grid_min.a_r] << "," << algorithms[grid_min.a_c] << ") -> " << grid_min.t << "}" << endl;
   }
